                ...
                if (strlen($pseudo) <= 30) {
                    ...
                    if (strlen($email) <= 100) {
                        ...
                        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            $password = hash('sha256', $password);
                            $insert = $this->pdo->prepare("INSERT INTO user(user_pseudo,user_firstName, user_lastName, user_email, user_password) 
                                                             VALUES (:pseudo, :prenom, :nom, :email, :pass)");
                            ...
                            header('location: signInView.php?error=email');
                        }
                    } else {
                        header('location: signInView.php?error=emailLenght');
                    }
                } else {
                    header('location: signInView.php?error=pseudo');
                }
            } else {
                header('location: signInView.php?error=already');
            }
        }
    }
